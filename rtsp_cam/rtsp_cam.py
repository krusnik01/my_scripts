...
# Send_foto
def send_tg(name):
    files = {'photo': open(f"./log/full.jpg", 'rb')}
...
def read_dic():
    fail_conunt={}
    try:
        with open('./log/fail_conunt') as inp:
...
def wr_dic(fail_conunt):
    with open('./log/fail_conunt', 'w') as out:
...
def compare_pixel(name):
    ...
    try:
        new=Image.open('./log/'+name)
    except FileNotFoundError as err:
        wr_log(f'ФАЙЛ {err.filename} НЕ НАЙДЕН \n')
        return
    stok_load=stok.load()
    new_load=new.load()
    x1,y1 = stok.size
    ...
    for x in range(0,x1):
        for y in range(0,y1):
            ...
            if compare_num>50:
                i+=1
    if i>=800 and fail_conunt[name]==0:
        send_tg(name)
        fail_conunt[name]+=1
    elif i>=800 and fail_conunt[name]>0:
...
while(True):
    ...
    ret, frame = cap.read()
    if ret:
        cv2.imwrite(f'./log/full.jpg',frame)
        for name in img_name.keys():
            cv2.imwrite(f'./log/{name}',frame[int(img_name[name][0]):int(img_name[name][1]),int(img_name[name][2]):int(img_name[name][3])]) #выглядит страшно но так надо
            compare_pixel(name)
    else:
